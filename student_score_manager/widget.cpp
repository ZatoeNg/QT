...
void widget::on_SearchEdit_editingFinished()
{
    onSearch();
    qDebug() << __FUNCTION__;
}

void widget::loadTestData(const QString& filename)
{
    //打开文件
    QFile file(filename);
    ...
    auto s =line.split(QRegularExpression("[ \t]"));
    qDebug()<<"one:"<<s<<Qt::endl;
